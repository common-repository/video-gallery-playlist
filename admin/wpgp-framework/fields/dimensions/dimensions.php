	...
	class WPGP_Field_dimensions extends WPGP_Fields {
		...
		public function render() {
			...
			if ( ! empty( $args['width'] ) ) {
				...
				echo '<input type="number" name="' . esc_attr( $this->field_name( '[width]' ) ) . '" value="' . esc_attr( $value['width'] ) . '"' . $placeholder . ' class="wpgp-input-number' . esc_attr( $is_unit ) . '" step="any" />';
				echo ( ! empty( $unit ) ) ? '<span class="wpgp--label wpgp--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
				echo '</div>';
			}
			...
			if ( ! empty( $args['height'] ) ) {
				...
				echo '<input type="number" name="' . esc_attr( $this->field_name( '[height]' ) ) . '" value="' . esc_attr( $value['height'] ) . '"' . $placeholder . ' class="wpgp-input-number' . esc_attr( $is_unit ) . '" step="any" />';
				echo ( ! empty( $unit ) ) ? '<span class="wpgp--label wpgp--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
				echo '</div>';
			}
