 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'WPGP_Field_select' ) ) {
	class WPGP_Field_select extends WPGP_Fields {
		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( ! empty( $args['ajax'] ) ) {
					...
					$args['settings']['data']['nonce'] = wp_create_nonce( 'wpgp_chosen_ajax_nonce' );
				...
				$chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' wpgp-chosen-sortable' : '';
				$chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' wpgp-chosen-ajax' : '';
				$placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '"' : '';
				$field_class      = ( $args['chosen'] ) ? ' class="wpgp-chosen' . esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) . '"' : '';
				...
				if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
					...
					if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
						...
						echo '<select name="' . esc_attr( $field_name ) . '" class="wpgp-hide-select hidden"' . wp_kses_post( $multiple_attr . $field_attr ) . '>';
